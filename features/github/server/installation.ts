...
import {getGithubApp, getGithubInstallUrl} from "@/features/github/utils/github-app";
...
export async function uninstallGithubApp(userId: string) {
  const installation = await prisma.githubInstallation.findUnique({
    where: { userId },
  });
...
export async function getInstallationIdByUserId(userId: string) {
    const installation = await prisma.githubInstallation.findUnique({
        where: { userId },
